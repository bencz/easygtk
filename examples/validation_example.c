/**
 * EasyGTK - Validation Example
 *
 * Demonstrates the declarative validation system:
 * - Built-in validators (required, min/max length, email, pattern, range)
 * - Custom validator
 * - Automatic validation on focus out
 * - Batch validation
...
static EgValidationResult validate_password_match(const EgValidationValue *value, void *user_data) {
    ...
    if (confirm && password && strcmp(confirm, password) != 0) {
        ...
        result.error_message = "Passwords do not match";
    }
...
static EgValidationResult validate_strong_password(const EgValidationValue *value, void *user_data) {
    ...
    if (!has_upper || !has_lower || !has_digit) {
        result.is_valid = false;
        result.error_message = "Password must have uppercase, lowercase and number";
    }
...
static void on_validate_all(EgWidget *widget, void *user_data) {
    ...
    if (all_valid) {
        eg_label_set_markup(label_status,
            "<span color='#a6e3a1' weight='bold'>All fields are valid!</span>");
        ...
        if (first_invalid >= 0) {
            ...
            snprintf(msg, sizeof(msg),
                "<span color='#f38ba8'>Field %d invalid: %s</span>",
                first_invalid + 1,
                results[first_invalid].error_message ? results[first_invalid].error_message : "error");
...
static void setup_validators(void) {
    ...
    eg_validator_add_required(name_chain, "Name is required");
    eg_validator_add_min_length(name_chain, 3, "Name must have at least 3 characters");
    eg_validator_add_max_length(name_chain, 50, "Name must have at most 50 characters");
    ...
    eg_validator_add_required(email_chain, "Email is required");
    eg_validator_add_email(email_chain, "Invalid email");
    ...
    /* Password: required, minimum 8 chars, must be strong */
    EgValidatorChain *password_chain = eg_validator_chain_new();
    eg_validator_add_required(password_chain, "Password is required");
    eg_validator_add_min_length(password_chain, 8, "Password must have at least 8 characters");
    ...
    eg_validator_add_required(confirm_chain, "Confirmation is required");
    ...
    eg_validator_add_range_double(age_chain, 18.0, 120.0, "Age must be between 18 and 120 years");
    ...
    /* Phone: pattern (optional - accepts empty) */
    EgValidatorChain *phone_chain = eg_validator_chain_new();
    /* Pattern: format (XX) XXXXX-XXXX or empty */
...
static void on_activate(EgWidget *widget, void *user_data) {
    ...
    /* Main window */
    EgWindow *window = eg_window_new(app, "Form Validation", 500, 550);
    ...
    eg_label_set_markup(title, "<span size='xx-large' weight='bold'>User Registration</span>");
    eg_widget_set_halign(eg_label_as_widget(title), EG_ALIGN_CENTER);

    EgLabel *subtitle = eg_label_new("Fields are validated on focus out");
    ...
    entry_name = eg_entry_new_with_placeholder("Your full name");
    ...
    entry_email = eg_entry_new_with_placeholder("user@example.com");
    ...
    /* Phone */
    EgLabel *lbl_phone = eg_label_new("Phone");
    ...
    /* Legend */
    EgLabel *legend = eg_label_new("* Required fields");
    ...
    EgButton *btn_validate = eg_button_new("Validate and Submit");
    ...
    /* Setup validators AFTER creating widgets */
    setup_validators();
